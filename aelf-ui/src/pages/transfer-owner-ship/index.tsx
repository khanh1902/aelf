...
const TransferOwnerShip = ({ provider, currentWalletAddress }: { provider: IPortkeyProvider | null; currentWalletAddress: string }) => {
  ...
  const onSubmit = async () => {
    const transferOwnerShipLoadingId = toast.loading('Transfer Owner Ship Executing');

    try {
      const input = {
        address,
      };
      await sideChainSmartContract?.callSendMethod('TransferOwnership', currentWalletAddress, input);

      toast.update(transferOwnerShipLoadingId, {
        render: 'Transfer Owner Ship Successfully!',
        type: 'success',
        isLoading: false,
      });
      removeNotification(transferOwnerShipLoadingId);
    ...
    <div className='form-wrapper'>
      <div className='form-container'>
        <div className='form-content'>
          <h2 className='form-title'>TransferOwnership</h2>
